...
parallel_copy(size_t grainsize, InputIterator first, InputIterator last, OutputIterator result) {
    tbb::parallel_for(tbb::blocked_range<InputIterator>(first, last, grainsize), [&](const auto& r) {
...
{
    tbb::parallel_for(tbb::blocked_range<InputIterator1>(first1, last1, grainsize), [&](const auto& r) {
