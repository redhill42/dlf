...
const std::vector<database::DatabaseEntry> Database::apple_cpu_fallback {
  database::XaxpyApple,
  database::XdotApple,
  database::XgemvApple,
  database::XgemvFastApple,
  database::XgemvFastRotApple,
  database::XgerApple,
  database::XtrsvApple,
  database::XgemmApple,
  database::XgemmDirectApple,
  database::CopyApple,
  database::PadApple,
  database::TransposeApple,
  database::PadtransposeApple,
  ...
  : parameters_(std::make_shared<database::Parameters>())
{
  ...
  // Special case: modifies the database if the device is a CPU with Apple OpenCL
    ...
    if (device.type() == DeviceType::CPU) {
      const auto is_apple = device.hasExtension("cl_APPLE_SetMemObjectDestructor");
...
database::Parameters Database::Search(
    const std::string& this_kernel,
    const std::string& this_vendor,
    const std::string& this_type,
    const std::string& this_device,
    const std::string& this_architecture,
    ...
    const std::vector<database::DatabaseEntry>& this_database) const
{
  ...
  for (auto& db : this_database) {
    if ((db.kernel == this_kernel) && (db.precision == this_precision || db.precision == Precision::Any)) {
      // Searches for the right vendor and device type, or selects the default if unavailable
      const auto parameters = SearchVendorAndType(
        this_vendor, this_type, this_device, this_architecture,
        db.vendors, db.parameter_names);
      if (parameters.size() != 0)
        return parameters;
      return SearchVendorAndType(
        kDeviceVendorAll, database::kDeviceTypeAll, this_device, this_architecture,
        db.vendors, db.parameter_names);
    }
  }
...
database::Parameters Database::SearchVendorAndType(
  const std::string& target_vendor,
...
database::Parameters Database::SearchArchitecture(
...
database::Parameters Database::SearchDevice(
  ...
  for (auto& device : devices) {
    ...
    if (device_name == target_device_cut_off) {
      ...
      if (parameter_names.size() > device.parameters.size())
        return database::Parameters(); // ERROR
