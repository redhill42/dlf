...
template <typename T>
Xsub<T>::Xsub(const Queue& queue, Event* event, const std::string& name)
    : Routine(queue, event, name, {"Xaxpy"}, PrecisionValue<T>(), {}, {
    #include "../../kernels/level1/level1.cl"
    #include "../../kernels/dnn/xsub.cl"
    #include "../../kernels/dnn/xtransform2.cl"
    }) {
}

template <typename T>
void Xsub<T>::DoSub(const size_t x_size, const Buffer<T>& x_buffer, const size_t x_offset, const size_t x_inc,
                    ...
                    Buffer<T>& z_buffer, const size_t z_offset, const size_t z_inc)
{
    ...
    const auto kernel_name = (use_fastest_kernel) ? "XsubFastest" :
                             (use_faster_kernel)  ? "XsubFaster" : "Xsub";
...
template class Xsub<int16_t>;
template class Xsub<int32_t>;
template class Xsub<int64_t>;
template class Xsub<half>;
template class Xsub<float>;
template class Xsub<double>;
template class Xsub<float2>;
template class Xsub<double2>;
