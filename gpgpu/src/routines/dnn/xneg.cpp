...
template <typename T>
Xneg<T>::Xneg(const Queue& queue, Event* event, const std::string& name)
    : Routine(queue, event, name, {"Xaxpy"}, PrecisionValue<T>(), {}, {
    #include "../../kernels/level1/level1.cl"
    #include "../../kernels/dnn/xneg.cl"
    }) {
}
    ...
    Buffer<T>& y_buffer, const size_t y_offset, const size_t y_inc)
{
    ...
    auto kernel = program_.getKernel("Xneg");
...
template class Xneg<int16_t>;
template class Xneg<int32_t>;
template class Xneg<int64_t>;
template class Xneg<half>;
template class Xneg<float>;
template class Xneg<double>;
