...
template <typename T>
Xsign<T>::Xsign(const Queue& queue, Event* event, const std::string& name)
    : Routine(queue, event, name, {"Xaxpy"}, PrecisionValue<T>(), {}, {
    #include "../../kernels/level1/level1.cl"
    #include "../../kernels/dnn/xsign.cl"
    }) {
}
    ...
    Buffer<T>& y_buffer, const size_t y_offset, const size_t y_inc)
{
    ...
    auto kernel = program_.getKernel("Xsign");
...
template class Xsign<int16_t>;
template class Xsign<int32_t>;
template class Xsign<int64_t>;
template class Xsign<half>;
template class Xsign<float>;
template class Xsign<double>;
