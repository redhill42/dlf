...
template <typename T>
Xmul<T>::Xmul(const Queue& queue, Event* event, const std::string& name)
    : Routine(queue, event, name, {"Xaxpy"}, PrecisionValue<T>(), {}, {
    #include "../../kernels/level1/level1.cl"
    #include "../../kernels/dnn/xmul.cl"
    #include "../../kernels/dnn/xtransform2.cl"
    }) {
}

template <typename T>
void Xmul<T>::DoMul(const size_t x_size, const Buffer<T>& x_buffer, const size_t x_offset, const size_t x_inc,
                    ...
                    Buffer<T>& z_buffer, const size_t z_offset, const size_t z_inc)
{
    ...
    const auto kernel_name = (use_fastest_kernel) ? "XmulFastest" :
                             (use_faster_kernel)  ? "XmulFaster" : "Xmul";
...
template class Xmul<int16_t>;
template class Xmul<int32_t>;
template class Xmul<int64_t>;
template class Xmul<half>;
template class Xmul<float>;
template class Xmul<double>;
template class Xmul<float2>;
template class Xmul<double2>;
