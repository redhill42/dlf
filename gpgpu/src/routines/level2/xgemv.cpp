...
template <typename T>
Xgemv<T>::Xgemv(const Queue &queue, Event* event, const std::string &name):
    Routine(queue, event, name, {"Xgemv", "XgemvFast", "XgemvFastRot", "TrsvRoutine"}, PrecisionValue<T>(), {}, {
    #include "../../kernels/level2/xgemv.cl"
    #include "../../kernels/level2/xgemv_fast.cl"
    #include "../../kernels/level2/xtrsv.cl"
    }) {
}
...
void Xgemv<T>::MatVec(const Layout layout, const Transpose a_transpose,
  ...
  const auto a_altlayout = (layout == Layout::RowMajor);
  auto a_one = (a_altlayout) ? n : m;
  const auto a_two = (a_altlayout) ? m : n;

  // Swap m and n if the matrix is transposed
  const auto a_transposed = (a_transpose != Transpose::NoTrans);
  ...
  const auto a_conjugate = (a_transpose == Transpose::ConjTrans);
...
template class Xgemv<int32_t>;
template class Xgemv<int64_t>;
