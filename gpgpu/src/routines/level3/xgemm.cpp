...
void Xgemm<T>::DoGemm(const Layout layout,
                      ...
                      Buffer<T> *temp_buffer) { // optional arguments
...
void Xgemm<T>::GemmIndirect(const size_t m, const size_t n, const size_t k,
  ...
  const auto temp_buffer_all = temp_buffer   ? *temp_buffer :
                               temp_size > 0 ? context_.createBuffer<T>(temp_size)
                                             : a_buffer;

  // Verifies if the provided temporary buffer is large enough
  if (temp_buffer) {
    const auto required_size = temp_size * sizeof(T);
    if (temp_buffer_all.data_size() < required_size)
        throw BLASError(StatusCode::kInsufficientMemoryTemp);
  }

  // Sets the buffer pointers for (temp) matrices A, B, and C
  auto a_temp = (a_no_temp) ? a_buffer : temp_buffer_all;
  auto b_temp = (b_no_temp) ? b_buffer : temp_buffer_all;
  auto c_temp = (c_no_temp) ? c_buffer : temp_buffer_all;
