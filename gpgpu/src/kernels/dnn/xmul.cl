...
INLINE_FUNC realV MultiplyVectorVector(const realV aval, const realV bvec) {
  realV cvec;
  #if VW == 1
    ...
    __global real* zgm, const int z_offset, const int z_inc)
{
  ...
  for (int id = get_global_id(0); id < n; id += get_global_size(0)) {
    ...
    real z_value;
    Multiply(z_value, x_value, y_value);
    ...
    __global realV* zgm)
{
  if (get_global_id(0) < n / (VW)) {
    ...
    for (int _w = 0; _w < WPT; _w++) {
      ...
      zgm[id] = MultiplyVectorVector(xgm[id], ygm[id]);
    }
  }
}
