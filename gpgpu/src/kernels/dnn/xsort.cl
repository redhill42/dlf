...
INLINE_FUNC void LocalSort(const int i, const int L, const int dir, LOCAL_PTR real* lm) {
    for (int k = 2; k <= L; k <<= 1) {
        int ddd = dir ^ ((i & (k / 2)) != 0);
          ...
          __global int*  igm, int i_offset, const int i_inc)
{
    ...
    unravel3(batch*n + gid, &x_offset, &y_offset, &i_offset, rank, shape);
    xgm += x_offset;
    ygm += y_offset;
    igm += i_offset;
...
__kernel void DirectArgMerge(
    const int n, const int dir,
    const int rank, __constant int* shape1, __constant int* shape2,
    const __global real* restrict  xgm, int  x_offset, const int  x_inc,
    const __global int*  restrict ixgm, int ix_offset, const int ix_inc,
          __global real*           zgm, int  z_offset, const int  z_inc,
          __global int*           izgm, int iz_offset, const int iz_inc
#ifndef CUDA
    , __local real* lm) {
#else
    ) { extern __shared__ real lm[];
    ...
    unravel2(batch*n + gid,  &x_offset,  &z_offset, rank, shape1);
    unravel2(batch*n + gid, &ix_offset, &iz_offset, rank, shape2);
     xgm += x_offset;
    ixgm += ix_offset;
     zgm += z_offset;
    izgm += iz_offset;
    ...
    LocalArgMerge(dir,
                  x_len, xgm, x_inc, ixgm, ix_inc,
                  y_len, xgm + x_len*x_inc, x_inc, ixgm + x_len*ix_inc, ix_inc,
                  z_len, zgm, z_inc, izgm, iz_inc,
                  lm, lm + lsz + 1);
}

__kernel __attribute__((reqd_work_group_size(WGS, 1, 1)))
void IndirectArgMerge(
    const int n, const int k, const int dir,
    const int rank, __constant int* shape1, __constant int* shape2,
    const __global real* restrict  xgm, int  x_offset, const int  x_inc,
    const __global int*  restrict ixgm, int ix_offset, const int ix_inc,
          __global real*           zgm, int  z_offset, const int  z_inc,
